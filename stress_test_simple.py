#!/usr/bin/env python3
"""
DigitalHuman Stress Test - Simple but effective
Uses websockets with browser-like headers for realistic testing
"""
...
async def test_single_request(char_id: str, question: str) -> dict:
    """Test a single request to the server"""
    import websockets

    uri = f"ws://ec2-13-114-64-14.ap-northeast-1.compute.amazonaws.com:8080/ws?char={char_id}"

    # Browser-like headers
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
        "Origin": "http://ec2-13-114-64-14.ap-northeast-1.compute.amazonaws.com:8080",
    }
        ...
        while not done:
            try:
                ...
                if isinstance(msg, str):
                    ...
                    elif data.get("type") == "status" and data.get("content") == "done":
                        done = True
...
async def run_stress_test(duration_minutes: int = 60):
    """Run stress test for specified duration"""
    results = StressTestResults()
    end_time = time.time() + (duration_minutes * 60)

    print(f"\n{'='*70}")
    print(f"🚀 DigitalHuman Stress Test")
    print(f"{'='*70}")
    ...
    while time.time() < end_time:
        ...
        try:
            # Test the request
            result = await test_single_request(char_id, question)

            if result["done"]:
                results.record_success(result["response_time_ms"])
                stats = results.get_stats()
                print(f"[{datetime.now().strftime('%H:%M:%S')}] ✅ #{results.total_requests} | "
                      f"{char_id[:15]:15} | {result['response_time_ms']:6}ms | "
                      f"Audio: {result['audio_chunks']:4} chunks | "
